...
def computeIntrons(exon_regions, logger=None, generated_by=None, verbose=False):
    ...
    dellist =[]
    for exon in exon_regions[1:]:
        for desckey in exon.desc.keys():
            if desckey not in exon_regions[0].desc.keys():
                dellist.append(desckey)
            else:
                if exon_regions[0].desc[desckey]!=exon.desc[desckey]:
                    dellist.append(desckey)

    exon_common_desc_keys = {}
    for key in exon_regions[0].desc.keys():
        if key not in set(dellist):
            exon_common_desc_keys[key]=exon_regions[0].desc[key]
